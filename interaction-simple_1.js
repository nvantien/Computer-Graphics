...
InteractionApp.prototype.init = function(param)
{
    ...
    comment.init({ id : Control.ID_COMMENT, icon : "https://raw.githubusercontent.com/tparisi/WebGLBook/master/images/icons/comment.png" });
    this.addObject(comment);
    this.controls.push(comment);

    // Create the Control and add it to our sim
    var info = new Control();
    info.init({ id : Control.ID_INFO, icon : "https://raw.githubusercontent.com/tparisi/WebGLBook/master/images/icons/info.png" });
    this.addObject(info);
    this.controls.push(info);

    // Create the Control and add it to our sim
    var clock = new Control();
    clock.init({ id : Control.ID_CLOCK, icon : "https://raw.githubusercontent.com/tparisi/WebGLBook/master/images/icons/clock.png" });
    this.addObject(clock);
    this.controls.push(clock);

    // Create the Control and add it to our sim
    var favorite = new Control();
    favorite.init({ id : Control.ID_FAVORITE, icon : "https://raw.githubusercontent.com/tparisi/WebGLBook/master/images/icons/favorite.png" });
    this.addObject(favorite);
    this.controls.push(favorite);
    
    // Create the Control and add it to our sim
    var help = new Control();
    help.init({ id : Control.ID_HELP, icon : "https://raw.githubusercontent.com/tparisi/WebGLBook/master/images/icons/help.png" });
    this.addObject(help);
